...
import { Ticket, Calendar, DollarSign, Gift, Pencil } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const [totalCortesias, setTotalCortesias] = useState(0);

  useEffect(() => {
    eventsService.getMainEvent().then(setEvent);
    adminPaymentsService.listAll().then((payments) => {
      // Receita = só pagamentos de verdade, aprovados. Cortesias (origem
      // "manual") não entram na conta — não teve dinheiro entrando ali.
      const aprovados = payments.filter((p) => p.status === 'aprovado');
      const pagos = aprovados.filter((p) => p.origem !== 'manual');
      setReceitaTotal(pagos.reduce((a, p) => a + p.valorTotal, 0));
      setTotalCortesias(aprovados.filter((p) => p.origem === 'manual').reduce((a, p) => a + p.quantidade, 0));
    });
  }, []);
            ...
            {event.lotes.map((lot) => (
              <div key={lot.id} className="flex items-center justify-between rounded-xl border border-white/10 p-3 text-sm">
                <div>
                  ...
                  <p className="text-xs text-white/40">{formatCurrency(lot.preco)} · {lot.quantidadeVendida}/{lot.quantidadeTotal} vendidos (inclui cortesias)</p>
                </div>
