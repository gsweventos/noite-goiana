...
Regra central de segurança: **o frontend nunca cria ingressos, nunca confirma pagamentos e nunca fala diretamente com a API do Mercado Pago.** Ele só chama o backend (`/backend`, hospedado no Vercel), que é o único lugar com o Access Token do Mercado Pago e com credenciais para escrever nas coleções sensíveis do Firestore (`tickets`, `payments`, `checkins`).
...
│       │   ├── mercadopago.ts        # Cliente do Mercado Pago
...
│       │   ├── webhook.ts                        # POST — confirmação do Mercado Pago
...
### 5.2 Criar conta no Mercado Pago Developers

1. [mercadopago.com.br/developers/panel](https://www.mercadopago.com.br/developers/panel)
2. Crie uma aplicação → **Pagamentos online** → **Checkout Pro**
3. Copie o **Access Token de produção** — é ele que vai virar a variável `MP_ACCESS_TOKEN`
4. Confirme também que sua conta tem uma **chave Pix cadastrada** (pelo app do Mercado Pago), senão o Pix não aparece como opção pro comprador
...
### 5.5 Webhook do Mercado Pago

Diferente do Asaas, o Mercado Pago **não exige cadastro manual de webhook** — a URL de notificação é enviada automaticamente em cada preference criada (campo `notification_url`, já configurado em `backend/api/payments/create-preference.ts` apontando para `PUBLIC_API_URL + /payments/webhook`). Não precisa fazer nada manualmente no painel.
...
4. Mercado Pago chama `POST /api/payments/webhook`
5. Backend **reconsulta a API do Mercado Pago** pelo id do pagamento (nunca confia só no corpo do webhook), confirma `status === "approved"`
...
- **Identificação completa do pagador**: CPF do comprador é enviado ao Mercado Pago junto com a preference — reduz rejeições do motor de risco deles
...
- **Nenhum segredo no frontend**: Access Token do Mercado Pago, `QR_SECRET` e credenciais do Firebase Admin existem só no backend (Vercel)
